/* Code aanpassen aan Rooptopia situatie */
/*--------------  coin_sales1 start ------------*/

var socket = io('https://roomtopia-emielpopelier.c9users.io');
var sensordata = {
    luchtdruk: new Array(),
    temperatuur: new Array(),
    hoogtepunt: new Array()
}

socket.on('connect', function () {
    socket.on('liveData', function (data) {
        sensordata.luchtdruk.push(data.luchtdruk);
        sensordata.temperatuur.push(data.temperatuur);
        sensordata.hoogtepunt.push(data.hoogtepunt);
        
        console.log(sensordata.temperatuur);
        $("#currentLuchtdrukValue").text(sensordata.luchtdruk[sensordata.luchtdruk.length-1]);
        $("#currentTemperatuurValue").text(sensordata.temperatuur[sensordata.temperatuur.length-1]);
        $("#currentHoogtepuntValue").text(sensordata.hoogtepunt[sensordata.hoogtepunt.length-1]);
        
        var luchtdrukChange = sensordata.luchtdruk[sensordata.luchtdruk.length-1] - sensordata.luchtdruk[sensordata.luchtdruk.length-2];
        var temperatuurChange = sensordata.temperatuur[sensordata.temperatuur.length-1] - sensordata.temperatuur[sensordata.temperatuur.length-2];
        var hoogtepuntChange = sensordata.hoogtepunt[sensordata.hoogtepunt.length-1] - sensordata.hoogtepunt[sensordata.hoogtepunt.length-2];
        
        var luchtdrukPrefix = (luchtdrukChange > 0) ? "+" : "";
        var temperatuurPrefix = (temperatuurChange > 0) ? "+" : "";
        var hoogtepuntPrefix = (hoogtepuntChange > 0) ? "+" : "";
    
        $("#luchtdrukChange").text(luchtdrukPrefix + luchtdrukChange);
        $("#temperatuurChange").text(temperatuurPrefix + temperatuurChange);
        $("#hoogtepuntChange").text(hoogtepuntPrefix + hoogtepuntChange);
    ...
    var chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: '#0b76b6',
                data: sensordata.temperatuur, //Weet niet of dit werkt maar wie weet?
            }]
        },
